...
function createTableBody() {
	...
	for (var i = 0; i < length; i++) {
		...
		if(user.history[length-i-1].changeToBalance.substring(0,1) == "-") {
			newRow += "<td class='red-text' style='text-align: right; padding-right: 40px;'>";
		} else if(user.history[length-i-1].changeToBalance == "$0.00") {
			newRow += "<td class='blue-text' style='text-align: right; padding-right: 40px;'>";
		} else {
			newRow += "<td class='green-text' style='text-align: right; padding-right: 40px;'>";
		}
